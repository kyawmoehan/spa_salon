...
function checkBtn(id) {
    ...
    $("#voucher-discount-crash").val(0);
    $("#voucher-paid").val(total);
}
...
function getVouchers() {
    ...
    $.each(getVoucherArray, function (i, item) {
        var newListItem =
            `<li class="check_customer_name list-group-item d-flex justify-content-between align-items-center pe-0 py-0">` +
            item["customerName"] +
...
function addItem(itemId, itemName, itemPrice) {
    ...
    if (getVoucherArray[index].hasOwnProperty("items")) {
        ...
        if (getItem) {
            const ItemIndex = getVoucherArray[index].items.findIndex(
                (object) => {
    ...
    if (getVoucherArray[index].hasOwnProperty("services")) {
        if (indexs.length != 0) {
            indexs.forEach(function (sindex, i) {
                ...
                getVoucherArray[index].services[sindex]["staffAmount"] =
                    staffAmount;
            });
        }
    }
...
function increaseStaffPct(getVoucherArray, index, serviceIndex) {
    ...
    let nameCheckbox =
        getVoucherArray[index].services[serviceIndex].nameCheckbox;
    ...
    let servicePrice =
        getVoucherArray[index].services[serviceIndex].servicePrice;
...
function editStaffAmount(serviceId, staffId) {
    ...
    getVoucherArray[index].services[serviceIndex]["staffAmount"] = changeAmount;
